...
class App
{
    ...
    /**
     * @var string $appPath	    Абсолютный путь к приложению
     */
	...
	public function run()
	{
        ...
        if(!isset(self::$config['appPath']) OR empty(self::$config['appPath']) OR !is_dir(ROOT.self::$config['appPath']))
            array_shift($parts);
        else{
            foreach($parts as $k=>$v){
                if($v != self::$config['appPath']){
                    unset($parts[$k]);
                }else{
                    unset($parts[$k]);
                    break;
        ...
        self::$appPath = ROOT.self::$config['appPath'].DS;
    ...
    private function runController()
    {
        ...
        if(file_exists($cPath))
        {
            ...
            // Проверка на существование actions
                ...
                $cationPath = ROOT.self::$config['appPath'].DS.$actions[self::$method].'.php';
	...
	{
        if(isset(self::$config['appPath'])){
	    	$appPath = self::$config['appPath'];
	    }else{
	        $appPath = 'App';
        }
        
        set_include_path(ROOT.$appPath.DS.'Classes');
		spl_autoload_extensions(".php");
    	spl_autoload($className);
	}
    ...
    private function modelsClasses($className)
	{
        ...
        set_include_path(ROOT.$appPath.DS.'Models');
		spl_autoload_extensions(".php");
    	spl_autoload($className);
	}
    ...
    public static function initLang($langCode=false, $cookie=false)
    {
        ...
        $file = App::$appPath.'Languages'.DS.$langCode.'.php';
    ...
    private function findUrl()
    {
        ...
        self::$urlTheme = "http://".$httpHostFull."/".self::$config["appPath"]."Views/layout";
	...
	{
		if (file_exists($fileName = ROOT.App::$config['appPath'].DS.$file.'.php')) {
